...
function downloadResults() {
	...
	var textToWrite = xmlToRes(resultsString, resultsString.length);
...
function loadEasyTrain() {
	...
	for (let j = 0; j < sections.length; j++) {
		...
		if (sections[j].getAttribute("type") == 'html') {
			...
			for (let m = 0; m < pairs.length; m++) {
				lefts.push('<div class="grid-item">'
					//+ "<pre>" + pairs[m].getElementsByTagName("left")[0].innerHTML.replace(/ /g, '&nbsp;') + "</pre>"
					+ pairs[m].getElementsByTagName("left")[0].innerHTML.replace(/ /g, '&nbsp;')
					+ '</div>');
				centers.push('<div class="grid-item" '
					+ ' data-status="' + btnName + '"'
					+ ' id="mat'
				...
				rights.push('<div class="grid-item" id="mat'
					...
					+ '<div draggable="true" ondragstart="drag(event)" id="dragmat'
					+ (j+1) + (m+1) + '">'
					//+ "<pre>" + pairs[m].getElementsByTagName("right")[0].innerHTML.replace(/ /g, '&nbsp;') + "</pre>"
					+ pairs[m].getElementsByTagName("right")[0].innerHTML.replace(/ /g, '&nbsp;')
					+ '</div></div>');
			}
			var nopairs = sections[j].getElementsByTagName("nopair");
			for (let n = 0; n < nopairs.length; n++) {
				...
				rights.push('<div class="grid-item" id="mat'
					...
					+ '<div draggable="true" ondragstart="drag(event)" id="dragmat'
					+ (j+1) + (n+1) + 'x">'
					//+ "<pre>" + nopairs[n].getElementsByTagName("right")[0].innerHTML.replace(/ /g, '&nbsp;') + "</pre>"
					+ nopairs[n].getElementsByTagName("right")[0].innerHTML.replace(/ /g, '&nbsp;')
					+ '</div></div>');
			}
			shuffleArray(rights);
			for (let k = 0; k < lefts.length; k++) {
